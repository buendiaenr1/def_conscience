...
def main():
    ...
    # Título
    plt.title(f'PCA de palabras más frecuentes (basado en {total_archivos} archivos)\n'
            f'PC1: {pca.explained_variance_ratio_[0]:.1%} var, PC2: {pca.explained_variance_ratio_[1]:.1%} var',
            fontsize=16, fontweight='bold', pad=20)
    plt.xlabel('Primera componente principal (PC1)', fontsize=14)
    plt.ylabel('Segunda componente principal (PC2)', fontsize=14)
    ...
    legend_text = "Clusters semánticos:\n"
    ...
    •	Consciousness AND artificial intelligence: La pregunta sobre si una máquina puede ser consciente es central en la filosofía de la IA.  (Chalmers, 1995).
    •	Phenomenology AND machine learning: ¿Puede un algoritmo "experimentar" algo? ¿O al menos modelar estructuras fenomenológicas? (McClelland, 2017)
    •	Qualia AND computational models: Los qualia son las cualidades subjetivas de la experiencia (ej. "lo que se siente ver el rojo"), explorar esta combinación permite investigar si existen modelos computacionales que intentan simular, explicar o incluso negar la relevancia de los qualia. (Chalmers, 1995)
    •	Intentionality AND AI: podemos adoptar una "postura intencional" hacia sistemas complejos, incluso si no tienen conciencia (Dennett D. C., 1990)
    •	Emotion AND neuroeducation: La neuroeducación estudia cómo el cerebro aprende, y la emoción es un modulador clave de la atención, la memoria y la motivación. Esta combinación busca literatura sobre cómo integrar el conocimiento neurocientífico de las emociones en entornos educativos (humanos o asistidos por IA) (Damasio A. , 1994) y computación afectiva (Picard, 1997)
    •	Global Neuronal Workspace AND simulation: La Teoría del Espacio de Trabajo Neuronal Global (GNWT) de Dehaene propone un modelo neurocientífico de la conciencia como "acceso global" a la información. Simular este modelo computacionalmente permite probar hipótesis sobre cómo podría emerger la conciencia en arquitecturas artificiales (Dehaene S. , 2014).
    •	Embodied cognition AND AI limitations: La cognición encarnada sostiene que la mente no es solo un software cerebral, sino que emerge de la interacción cuerpo-entorno (Varela, Thompson, & Rosch, 1991).
    •	Theory of Mind AND machine consciousness: La "teoría de la mente" es la capacidad de atribuir estados mentales a otros (Dennett D. , 1989).
    •	Neuroplasticity AND emotional modulation: La neuroplasticidad es la capacidad del cerebro de reorganizarse; las emociones modulan este proceso (ej. el estrés inhibe la plasticidad; la motivación la potencia) (Picard, 1997) en entornos educativos o de IA adaptativa, entender esta interacción permite diseñar sistemas que optimicen el aprendizaje afectivamente regulado (Pradeep, y otros, 2024). 
    •	Affective computing AND phenomenological gap: existe una "brecha fenomenológica": aunque una máquina simule emoción, no la siente (Picard, 1997).
    """
